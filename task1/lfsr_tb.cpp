...
int main (int argc, char **argv, char **env) {
    ...
    Vlfsr* top = new Vlfsr; //instantiate the counter module as Vlsfr - name of all generated files. This is the DUT
    //init trace dump
    Verilated::traceEverOn(true);
    VerilatedVcdC* tfp = new VerilatedVcdC; //vcdC is the data type we use for the trace file pointer
    top->trace (tfp, 99);
    tfp->open ("lfsr.vcd"); //dump the waveform data to lfsr.vcd
